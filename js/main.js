...
var boardSize = gLevels[currentLevel].SIZE;
...
function buildBoard(SIZE) {
  ...
  setRandomMines(gLevels[currentLevel].MINES);
  calculateNeighborsForAllBoard(boardSize);
...
function generateRandomCoords() {
  ...
  var i = rndInt(boardSize - 1);
  var j = rndInt(boardSize - 1);
  return { i, j };
}
...
function calculateNeighborsForSingleCell(givenI, givenJ) {
  for (var i = givenI - 1; i <= givenI + 1; i++) {
    for (var j = givenJ - 1; j <= givenJ + 1; j++) {
      if (i < 0 || j < 0 || j > boardSize - 1 || i > boardSize - 1) continue;
